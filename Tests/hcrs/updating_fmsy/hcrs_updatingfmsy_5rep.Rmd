...
sim_hcr <- function(b, r, r_s, error, p, k, years, hcr){ #hcr term is added into the model to run a cross list of a number of different hcr possibilities
  ...
  if(results$f_ratio_err[1] >= 2){
    ...
    if(results$f_ratio_err[1] > 1 & results$f_ratio_err[1] < 2){
       results$f[1] = hcr*f_int #Reduce by the hcr in the experiment (5%, 10%, 15%, etc.)
   }  
...
#Write as CSV:
library(here)
 write.csv(results_hcr, file=file.path(here(),"/Tests/hcrs/results_csv/hcrs_updatingfmsy_5rep.csv"),
           row.names = FALSE)
