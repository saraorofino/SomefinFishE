...
Assumptions:
 - The perceived Fmsy is constant (ie the managers are not updating their estimates of Fmsy during an assessment year)
...
check_result_5c <- sim_5c(b=6000, r=0.2, r_s=-0.01767, error=0.10, p=0.2, k=10000, years=100)
```
...
results_rep5c_edit <- results_rep5c %>%
  mutate(ratio_cat = case_when(results_rep5c$V7 <= 1 ~ 0,
                               results_rep5c$V7 > 1 & results_rep5c$V7 < 2 ~ 1,
                               results_rep5c$V7 > 2 ~ 2)) %>% 
  mutate(ratio_err_cat = case_when(results_rep5c$V8 <= 1 ~ 0,
                               results_rep5c$V8 > 1 & results_rep5c$V8 < 2 ~ 1,
                               results_rep5c$V8 > 2 ~ 2)) %>%
...
#           file=file.path(here(),"/Results/error/rep5_constant.csv"),
#           row.names = FALSE)
