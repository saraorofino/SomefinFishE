...
sim_5u <- function(b, r, r_s, error, p, k, years){
  ...
  m1 <- results$f_ratio[1] #mean of normal distribution
  s1 <- error * results$f_ratio[1] #stdev of normal distribution 
  mu1 <- log((m1^2) / sqrt((s1^2) + (m1^2))) #mean of lognorm
  sd1 <- sqrt(log(1 + ((s1^2) / (m1^2))))  #stdev of lognorm
  results$f_ratio_err[1] <- rlnorm(1, meanlog = mu1, sdlog = sd1) #draw the ratio with error from the lognormal distribution (should never be negative!)
  ...
  for (t in 2:years) {
    ...
    #Calculate the lognormal error - this requires some transformation of the original data, but can only be done if the f_ratio is greater than 0:
    if(results$f_ratio[t] != 0){
       #Assign values to normal dist mean (m) and stdev (s):
       m2 <- results$f_ratio[t]
       s2 <- error * results$f_ratio[t]
       #Calculate the lognormal mean(mu) and stdev(sd):
       mu2 <-  log((m2^2) / sqrt((s2^2) + (m2^2)))
       sd2 <- sqrt(log(1 + ((s2^2) / (m2^2))))
       #Draw the f_ratio_err:
       results$f_ratio_err[t] <- rlnorm(1, meanlog = mu2, sdlog = sd2)
    }
    if(results$f_ratio[t] == 0){
...
##Save CSV
Re-saved CSV file on 12/6 after fixing the error distribution equations (SO)
```{r save-csv}
...
#           file=file.path(here(),"/Results/error/error_updating_rep5.csv"),
#           row.names = FALSE)
