...
sim_10u <- function(b, r, r_s, error, p, k, years){
  ...
  m <- results$f_ratio[1] #mean of normal distribution
  s <- error * m #stdev of normal distribution 
  mu <- log(m^2 / sqrt(s^2 + m^2)) #log transformed mean
  sd <- sqrt(log(1 + (s^2 / m^2))) #log transformed stdev 
  results$f_ratio_err[1] <- rlnorm(1, meanlog = mu, sdlog = sd) #draw the ratio with error from the lognormal distribution (should never be negative!)
  ...
  for (t in 2:years) {
    ...
    #Calculate the lognormal error - this requires some transformation of the original data:
    m <- results$f_ratio[t] #mean of normal distribution
    s <- error * m #stdev of normal distribution 
    mu <- log(m^2 / sqrt(s^2 + m^2)) #log transformed mean
    sd <- sqrt(log(1 + (s^2 / m^2))) #log transformed stdev 
    results$f_ratio_err[t] <- rlnorm(1, meanlog = mu, sdlog = sd) #draw the ratio with error from the lognormal distribution 
...
#           file=file.path(here(),"/Results/error/error_updating_rep10.csv"),
#           row.names = FALSE)
