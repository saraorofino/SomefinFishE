...
err <- function(b, r, r_s, error, p, k, years){
  ...
  #Assign values to normal dist mean (m) and stdev (s):
  m1 <- results$f_ratio[1] 
  s1 <- error * results$f_ratio[1]
  
  #Transform to new mean (mu) and stdev (stdev)
  mu1 <- log((m1^2) / sqrt((s1^2) + (m1^2)))
  sd1 <- sqrt(log(1 + ((s1^2) / (m1^2)))) 
  
  #Draw the ratio:
  results$f_ratio_err[1] <- rlnorm(1, meanlog = mu1, sdlog = sd1)
