...
sim_20c <- function(b, r, r_s, error, p, k, years){
...
check_result_20c <- sim_20c(b=6000, r=0.2, r_s=-0.01767, error=0.10, p=0.2, k=10000, years=100)
```
...
for(i in 1:2295){
  results[[i]] <- sim_20c(b=input_20c[[i]]$b, r=input_20c[[i]]$r,
                             r_s=input_20c[[i]]$r_s, p=input_20c[[i]]$p,
                             k=input_20c[[i]]$k,years=input_20c[[i]]$years, 
                             error = input_20c[[i]]$error)
}
...
for(i in 1:2295){
  ...
  dat$id <- i
  dat$r_0 <- rep(input_20c[[i]]$r, 100)
  dat$err <- rep(input_20c[[i]]$error, 100)
  dat$r_s <- rep(input_20c[[i]]$r_s, 100)
  results_id[[i]] <- dat
}

#Bind all the experiments together
results_rep20c <- bind_rows(results_id)

#Mutate columns to compare f_ratio to f_ratio_err 
results_rep20c_edit <- results_rep20c %>%
  mutate(ratio_cat = case_when(results_rep20c$V7 <= 1 ~ 0,
                               results_rep20c$V7 > 1 & results_rep20c$V7 < 2 ~ 1,
                               results_rep20c$V7 > 2 ~ 2)) %>% 
  mutate(ratio_err_cat = case_when(results_rep20c$V8 <= 1 ~ 0,
                               results_rep20c$V8 > 1 & results_rep20c$V8 < 2 ~ 1,
                               results_rep20c$V8 > 2 ~ 2)) %>%
  mutate(correct_decision = case_when(ratio_cat == ratio_err_cat ~ "TRUE",
                                      ratio_cat != ratio_err_cat ~ "FALSE"))

#Update the column names:
colnames(results_rep20c_edit) <- col_names

#Write as CSV:
# library(here)
# write.csv(results_rep20c_edit,
#           file=file.path(here(),"/FISHE_tests/stock_assessment_new/catch_tests/csv_results/rep20_clim.csv"),
#           row.names = FALSE)
