...
experiment <- data.frame(
  r_s = runif(n_experiment, min = -0.3, max = 0.38)) #range of values from the table
...
for (i in 1:n_experiment){
  results_rs[[i]] <- test_rs(b = 0.6, f = 0.1, r = 0.2, r_s = experiment$r_s[i], p = 0.2,
                                years = 50, k = 1)
}
...
pdf(file = "G:/Data Analysis/SomefinFishE/Models/behavior_checks/rs/b_plots.pdf",
    width = 4,
    height = 4)
