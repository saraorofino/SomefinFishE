...
Goal: Simulate the initial use of FISHE, if it is only applied on 5 year intervals, focusing on the Inital Stock Assessment step of the process. 
...
sim_5 <- function(b, r, r_s, error, p, k, years){
...
check_result_5 <- sim_5(b=6000, r=0.2, r_s=0, error=0.10, p=0.2, k=10000, years=100)
```
...
for(i in 1:1320){
  results[[i]] <- sim_5(b=input_5[[i]]$b, r=input_5[[i]]$r,
                             r_s=input_5[[i]]$r_s, p=input_5[[i]]$p,
                             k=input_5[[i]]$k,years=input_5[[i]]$years, 
                             error = input_5[[i]]$error)
}
...
results_rep5 <- bind_rows(results_id)

#Update the column names:
colnames(results_rep5) <- col_names

#Write as CSV:
library(here)
write.csv(results_rep5, file=file.path(here(),"/FISHE_tests/stock_assessment_new/catch_tests/csv_results/rep5_noclim.csv"),
          row.names = FALSE)
