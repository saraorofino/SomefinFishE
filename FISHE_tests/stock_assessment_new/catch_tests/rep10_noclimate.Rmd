...
Goal: Simulate the initial use of FISHE, if it is only applied on 10 year intervals, focusing on the Inital Stock Assessment step of the process. 
...
sim_10 <- function(b, r, r_s, error, p, k, years){
...
check_result_10 <- sim_10(b=6000, r=0.2, r_s=0, error=0.10, p=0.2, k=10000, years=100)
```
...
for(i in 1:1320){
  results[[i]] <- sim_10(b=input_10[[i]]$b, r=input_10[[i]]$r,
                             r_s=input_10[[i]]$r_s, p=input_10[[i]]$p,
                             k=input_10[[i]]$k,years=input_10[[i]]$years, 
                             error = input_10[[i]]$error)
}
...
results_rep10 <- bind_rows(results_id)

#Update the column names:
colnames(results_rep10) <- col_names

#Write as CSV:
library(here)
write.csv(results_rep10, file=file.path(here(),"/FISHE_tests/stock_assessment_new/catch_tests/csv_results/rep10_noclim.csv"),
          row.names = FALSE)
