...
Goal: Simulate the initial use of FISHE, if it is only applied on 20 year intervals, focusing on the Inital Stock Assessment step of the process. 
...
sim_20 <- function(b, r, r_s, error, p, k, years){
...
check_result_20 <- sim_20(b=6000, r=0.2, r_s=0, error=0.10, p=0.2, k=10000, years=100)
```
...
for(i in 1:1320){
  results[[i]] <- sim_20(b=input_20[[i]]$b, r=input_20[[i]]$r,
                             r_s=input_20[[i]]$r_s, p=input_20[[i]]$p,
                             k=input_20[[i]]$k,years=input_20[[i]]$years, 
                             error = input_20[[i]]$error)
}
...
results_rep20 <- bind_rows(results_id)

#Update the column names:
colnames(results_rep20) <- col_names

#Write as CSV:
library(here)
write.csv(results_rep20, file=file.path(here(),"/FISHE_tests/stock_assessment_new/catch_tests/csv_results/rep20_noclim.csv"),
          row.names = FALSE)
