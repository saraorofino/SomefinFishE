...
sim_norep2 <- function(b, r, r_s, error, p, k, years){
  ...
  if(results$f_ratio_err[1] >= 2){
       f_act = 0 #Close the fishery
   } 
    if(results$f_ratio_err[1] > 1 & results$f_ratio_err[1] < 2){
       f_act = 0.9*f_int #Reduce by 10% - same thing as keeping 90% of previous fishing mortality
   }  
    if(results$f_ratio_err[1] == 1){
       f_act = f_int  #f stays the same in as last year
   }
    if(results$f_ratio_err[1] < 1){
       f_act = 1.05*f_int #f increases by 5% from last year
   } 
  
  #Calculate an f with biased error:
  results$f[1] = rnorm(1,mean = f_act, sd = (0.1*f_act)) #right now its drawing 10% error around f - want to bias this but I'm not sure how
  ...
  for (t in 2:years) {
    ...
    results$f[t] = rnorm(1,mean = f_act, sd = (0.1*f_act)) #randomly drawn f from the f_act in year 1
