...
Test 1: Initial Stock Assessment
...
Write the function to include error in the biomass estimates
```{r write_function}
#Write the function
sim_stockerror <- function(b, r, r_s, error, p, k, years){
  ...
  results$b[1] = b
  results$b_err[1] = rnorm(1, mean = results$b[1], sd = (error*results$b[1]))
...
##Tests1-4
Apply the new function to a set of experiments that vary actual initial biomass, where catch is responding to underlying changes in actual biomass but with some error
...
# for(i in input){
#   error_input[i] <- rep(input[i], 10)
# }
# We can't get the error term to repeat how we want it. Trying to get a sequence of 0.1 ten times, then 0.15 ten times etc. to apply each error over the full range of biomasses.
