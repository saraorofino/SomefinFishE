...
sim_hcr <- function(b, r, r_s, error, p, k, years, hcr){
  ...
  if(results$f_ratio_err[1] >= 2){
    ...
    if(results$f_ratio_err[1] > 1 & results$f_ratio_err[1] < 2){
       results$f[1] = hcr*f_int #hcr = range of reductions to f
...
check_result_hcr <- sim_hcr(b=6000, r=0.2, r_s=-0.005, error=0.10, p=0.2, k=10000, years=100, hcr=0.95)
```
...
#Write as CSV:
# library(here)
# write.csv(results_rep20c, file=file.path(here(),"/FISHE_tests/stock_assessment_new/catch_tests/csv_results/rep20_clim.csv"),
 #          row.names = FALSE)
