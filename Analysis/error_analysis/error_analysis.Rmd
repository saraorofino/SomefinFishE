...
error_slow <- err_rep5u %>% 
  filter(id == 413) %>% 
  filter(year < 36)

yield_error_slow = sum(error_slow$c)
...
biomass_slow <- ggplot(perfect_slow, aes(x=year, y=b)) + 
  ...
  geom_line(data =error_slow, aes(x=year, y=b, color = "c")) +
  ...
  scale_y_continuous(expand = c(0,0), limits = c(3000,4100), breaks = seq(3000,4100,200)) +
...
catch_slow <- ggplot(perfect_slow, aes(x=year, y=c)) + 
  ...
  geom_vline(xintercept = 34, color = "grey25", linetype = "longdash") +
  scale_color_manual(name = '', breaks = c("a","b", "c"), values = c('darkgreen', 'navy','firebrick4'),
                     labels = c("Perfect Management", "Perfect with FISHE", "FISHE with Error")) +
...
catch_fast <- ggplot(perfect_fast, aes(x=year, y=c)) + 
  ...
  geom_vline(xintercept = 19, color = "grey25", linetype = "longdash") +
  scale_color_manual(name = '', breaks = c("a","b", "c"), values = c('darkgreen', 'navy','firebrick4'),
                     labels = c("Perfect Management", "Perfect with FISHE", "FISHE with Error")) +
