...
Changing directions a bit - I want to compare perfect management to perfect given FISHE to some of these actual simulations. 
Find the simulations under perfect management, perfect given FISHE, and error 0.3 rep 5 that correspond to these initial conditions:
  - b0 = 4000
  - r = 0.2
  - r_s = -0.01767

Perfect Management: ID = 8
Perfect given FISHE: ID = 6
Error: ID = 413
```{r cumulative-yield}
...
yield_perfect = sum(perfect_sub$c)

#Calculate cumulative yield under perfect management given FISHE
fishe_sub <- perfect_fishe %>% 
  filter(id == 5)

yield_fishe = sum(fishe_sub$c)

#Calculate cumulative yield in error simulation
error_sub <- err_rep5u %>% 
  filter(id == 413)

yield_error = sum(error_sub$c)

##This shows that perfect given FISHE has higher yields than perfect management which doesn't make sense. But I think this is because perfect management doesn't have assessment intervals and both the error and the perfect given fishe are using assessment intervals which means they aren't exactly comparing apples to apples
```
