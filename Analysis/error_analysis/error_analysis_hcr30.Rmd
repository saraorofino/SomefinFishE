...
```{r slow-yield-calcs, include=FALSE}
...
slow_1 <- err_rep5u %>%
  ...
  filter(r_0 <= 0.3) %>% 
  filter(error == 0.1) %>% 
  filter(year == 100 & c!= 0) %>% #filter out experiments that don't make it to the end
  group_by(id) %>% 
  summarize(cumulative_yield = sum(c)) 
...
```
NOTE: There's a pretty big difference in the percent of perfect between keeping only simulations that don't fail by year 100 and keeping all simulations. Right now the numbers are only for simulations that don't fail by year 100, which are lower than the numbers including all the simulations (since some of them have at least some catch before they fail it skews the average up)   
